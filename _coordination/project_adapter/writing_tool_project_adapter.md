...
當前 Git state 以 `_coordination/current_state.md` §5 為準（single source of truth；Adapter 是規則層，state 不在這裡維護）。

預定 remote URL：`https://github.com/mark75369/writing-tool-project-folder.git`（rule-level，固定值；實際設定狀況見 current_state.md §5）。
...
- 各平台須各自填寫 Platform Capability Hook（暫定位置：未來新增的 Adapter section，或另立 `_coordination/platform_capability/` 檔案）
...
git 狀態查詢由 user 在 Windows PowerShell 執行並回貼（依 §7a，sandbox-git 不可信）。
...
git 狀態查詢由 user 在 Windows PowerShell 執行並回貼（依 §7a）。

Codex 在 pre-approved 下不可以做：

- 寫任何檔到 D:\writing-tool project folder（含任一子層）
- 從 sandbox 執行任何 git 指令（含 git status / git log / git diff 等 read-only；詳見 §7a）
- 執行任何非 read-only 腳本
